...
from modules.UGPIG import Recommender
...
if __name__ == '__main__':
    ...
    """build dataset"""
    train_cf, test_cf, user_dict, n_params, graph, mat_list,triplets,relation_dict = load_data(args)
    ...
    """define model"""
    model = Recommender(n_params, args, graph, mean_mat_list[0],'attention').to(device)
    ...
    for epoch in range(args.epoch):
        ...
        if  epoch==34:
            ...
            user_list_batch=range(2596)
            user_batch = torch.LongTensor(np.array(user_list_batch)).to(device)
            item_batch=range(94)
            item_batch = torch.LongTensor(np.array(item_batch)).to(device)
            ...
            score=model.rating(u_g_embeddings, i_g_embddings)
            h=open("socre.txt","w",encoding='utf-8')
            h.write(str(score.tolist()))
            print(score)
    ...
    print('early stopping at %d, recall@3:%.4f' % (epoch, cur_best_pre_0))
