...
class Aggregator(nn.Module):
    ...
    def forward(self, entity_emb, user_emb, latent_emb,
                ...
                weight, disen_weight_att,item_emb_last):
        ...
        item_re_emb=entity_emb
        zerolike=torch.zeros_like(entity_emb)

        for i in range(94):
            zerolike[i]=item_emb_last(torch.tensor(i).to("cuda:0"))
        # print(zerolike)
        # print(item_emb_last(torch.tensor(0).to("cuda:0")).shape)
        ...
        score = nn.Softmax(dim=1)(score_).unsqueeze(-1)  # [n_users, n_factors, 1]   #   user p 矩阵

        """user aggregate"""
        
        # print(interact_mat.shape)
        # print(entity_emb.shape)
        user_agg = torch.sparse.mm(interact_mat, zerolike)  # [n_users, channel]
        ...
        return entity_agg, user_agg,item_re_emb
...
class GraphConv(nn.Module):
    ...
    def __init__(self, channel, n_hops, n_users,
                 ...
                 ind, item_emb_last,node_dropout_rate=0.5, mess_dropout_rate=0.1,):
        ...
        self.ind = ind
        self.item_emb_last=item_emb_last
    ...
    def forward(self, user_emb, entity_emb, latent_emb, edge_index, edge_type,
        ...
        for i in range(len(self.convs)):
            entity_emb, user_emb,item_re_emb = self.convs[i](entity_emb, user_emb, latent_emb,
                                                 edge_index, edge_type, interact_mat,
                                                 self.weight, self.disen_weight_att,self.item_emb_last)
        ...
        return entity_res_emb, user_res_emb, cor, item_re_emb


class Recommender(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat,agg):
    ...
    def _init_model(self):
        return GraphConv(channel=self.emb_size,
                         ...
                         ind=self.ind,
                         item_emb_last=self.item_emb_last,
    ...
    def forward(self, batch=None):
        ...
        # print(self.all_embed.shape,user_emb.shape,entity_emb.shape)
        # entity_gcn_emb: [n_entity, channel]
        # user_gcn_emb: [n_users, channel]
        entity_gcn_emb, user_gcn_emb, cor ,item_re_emb= self.gcn(user_emb,
                                                     entity_emb,
                                                     self.latent_emb,
                                                     self.edge_index,
                                                     self.edge_type,
        ...
        # print(user.shape)
        # print(user_gcn_emb.shape,entity_gcn_emb.shape)
        # u_e = user_gcn_emb[user]+entity_gcn_emb[user]      !!!1
        if self.agg=='attention':
            a=F.relu(self.w(user_gcn_emb[user]))
            b=F.relu(self.w(entity_gcn_emb[user]))
   
            # with torch.no_grad():
            c=torch.cat((a,b),1)
            c=self.softmax(c)

            u_e = c[:,0].reshape(c[:,0].shape[0],1)*user_gcn_emb[user]+c[:,1].reshape(c[:,1].shape[0],1)*entity_gcn_emb[user]
        else:
            u_e = user_gcn_emb[user]+entity_gcn_emb[user]
            
        
        
        
        # print(u_e.shape)
        # print(self.item_emb_last(torch.tensor([1]).to(self.device)))
    ...
    def generate(self):
        ...
        return *self.gcn(user_emb,
                        ...
                        self.edge_type,
                        self.interact_mat,
                        mess_dropout=False, node_dropout=False)[:],self.item_emb_last,self.w #修改
